...
class Sanitizers {
   ...
   */
  static htmlSanitize = (string) => string.replace(/&/gu, '&amp;')
    .replace(/</gu, '&lt;')
    .replace(/>/gu, '&gt;')
    .replace(/"/gu, '&quot;')
    .replace(/'/gu, '&#x27;')
   ...
   */
  static htmlSanitizeArray = (array) => array.map((value) => {
   ...
   * @return {Object}     Sanitized object.
   */
  static htmlSanitizeObject = (obj) => Object.entries(obj).reduce((acc, [key, value]) => {
    if (typeof value === 'string') {
      return {
        ...acc,
